  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      if (!eventsError) {
        ...
        setTodayEvents(todayLocalEvents);
      }
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {todayEvents.map((event) => {
                const isSyncedEvent = event.source === 'calendar_sync';
                return (
                  <div
                    key={event.id}
                    className="p-3 sm:p-4 rounded-xl shadow-sm hover:shadow-md transition-shadow cursor-pointer bg-white border border-gray-100"
                    onClick={() => onNavigate('calendar')}
                  >
                    <div className="flex items-start space-x-3">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center flex-shrink-0 bg-rose-100">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center space-x-2 flex-1">
                            ...
                            {isSyncedEvent && (
                              <div className="flex items-center space-x-1 px-2 py-0.5 bg-blue-50 rounded-full border border-blue-200">
                                <Calendar className="w-3 h-3 text-blue-600" />
                                <span className="text-xs font-medium text-blue-600">Synced</span>
                              </div>
                            )}
                          </div>
                          <span className="text-xs sm:text-sm font-medium text-rose-600">{formatEventTime(event.start_time)}</span>
                        </div>
                        {event.location && (
